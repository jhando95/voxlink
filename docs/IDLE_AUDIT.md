...
| `tick_loop/mod.rs:206` | Keyboard poll (`device_query`) | 10–40 Hz | Read OS key state for PTT, mute, deafen, quick switcher | medium | ~~THROTTLE~~ ✓ DONE (v0.13.5) | In-call polling now drops to the 10 Hz out-of-call rate unless push-to-talk is the mic mode, a keybind is being captured, or the quick switcher is open; skipped ticks reuse the previous OS sample so edge detection never sees a phantom release. PTT keeps the full 40 Hz for press/release latency |
