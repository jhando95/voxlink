...
pub fn keycode_to_config_name(key: Keycode) -> &'static str {
    match key {
        ...
        Keycode::LControl => "lcontrol", Keycode::RControl => "rcontrol",
        Keycode::LAlt => "lalt", Keycode::RAlt => "ralt",
        Keycode::LMeta => "lmeta", Keycode::RMeta => "rmeta",
        _ => "unknown",
    }
}
...
pub fn keycode_to_display_name(key: Keycode) -> &'static str {
    match key {
        ...
        Keycode::Delete => "Delete",
        Keycode::Grave => "`",
        Keycode::LShift => "Shift", Keycode::RShift => "R.Shift",
        Keycode::LControl => "Ctrl", Keycode::RControl => "R.Ctrl",
        Keycode::LAlt => "Alt", Keycode::RAlt => "R.Alt",
        Keycode::LMeta => "Cmd", Keycode::RMeta => "R.Cmd",
        _ => "Unknown",
    }
}
...
mod tests {
    ...
    fn config_name_round_trip() {
        let keys = [Keycode::Space, Keycode::M, Keycode::D, Keycode::LShift, Keycode::F5];
        for &key in &keys {
    ...
    fn combo_held_check() {
        let combo = parse_combo("lshift+m");
        assert!(combo_held(&combo, &[Keycode::LShift, Keycode::M, Keycode::A]));
