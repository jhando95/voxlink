...
pub(crate) use relay::audio::{relay_audio, relay_audio_udp};
...
fn screen_chunk_is_plausible(
    ...
    if chunk_len > shared_types::MAX_UDP_SCREEN_CHUNK_SIZE {
        return false;
    }
    let max_chunks =
        shared_types::MAX_SCREEN_FRAME_SIZE.div_ceil(shared_types::MAX_UDP_SCREEN_CHUNK_SIZE);
...
async fn relay_screen(state: &State, metrics: &Metrics, sender_id: &str, data: &[u8]) {
    ...
    send_screen_frame_to_peers(
        ...
        others,
        &frame,
        data.len() <= shared_types::MAX_UDP_SCREEN_CHUNK_SIZE,
    )
    .await;
}

async fn relay_screen_chunk(state: &State, metrics: &Metrics, sender_id: &str, data: &[u8]) {
    if sender_id.len() > u8::MAX as usize {
        return;
    }

    let Some((metadata, chunk_data)) = shared_types::decode_screen_chunk_metadata(data) else {
        return;
    };
...
async fn relay_screen_udp(
    ...
    let send_timeout = std::time::Duration::from_millis(300);
    let udp_frame_ok = data.len() <= shared_types::MAX_UDP_SCREEN_CHUNK_SIZE;
