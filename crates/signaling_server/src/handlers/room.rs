...
pub async fn collect_room_others(state: &State, room_code: &str, exclude_id: &str) -> Vec<Arc<Peer>> {
...
pub async fn broadcast_to_room(state: &State, room_code: &str, exclude_id: &str, msg: &SignalMessage) {
...
pub async fn handle_create_room(state: &State, peer_id: &str, user_name: String, password: Option<String>) {
                ...
                if room.peer_ids.len() >= MAX_ROOM_PEERS {
                    if let Some(peer) = s.peers.get(peer_id).cloned() {
                        ...
                        send_to(
                            &peer,
                            &SignalMessage::Error {
                                message: format!("Room is full (max {} participants)", MAX_ROOM_PEERS),
                            },
                        )
                        .await;
                    }
